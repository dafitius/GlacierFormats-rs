...
fn write_all_text_texd_in_game(glacier_game: rpkg_rs::WoaVersion, game_paths: GamePaths) {
    ...
    for partition in &package_manager.partitions {
        ...
        for (resource, id) in partition.latest_resources() {
            if (resource.data_type() == "TEXT") {
                ...
                if let Ok(mut texture_map) = TextureMap::read_le_args(&mut stream, (GlacierGame::from(glacier_game),)) {
                    if let Some(texd_ref) = resource.references().get(0) {
                        ...
                        texture_map.set_mipblock1(MipblockData::from_memory(&texd_data, glacier_game.into()).unwrap());
                    }
                    let packer = TextureMapBuilder::from_texture_map(&texture_map).unwrap();
                    let texture = packer.build(glacier_game.into()).unwrap();
                    let vec = GlacierResource::serialize(&texture, glacier_game.into()).unwrap();
                    ...
                    if !texd_data.is_empty(){
                        ...
                        let rebuilt_texd = GlacierResource::serialize(&mipblock_data, glacier_game).unwrap();
