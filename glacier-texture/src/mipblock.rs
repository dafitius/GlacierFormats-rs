...
use crate::texture_map::{
    TextureMapError, TextureMapHeaderImpl, TextureMapHeaderV1, TextureMapHeaderV2,
};
...
impl MipblockData {
        ...
        let read_size = match version {
            GlacierGame::HM2016 => {
                ...
                stream
                    ...
                    .map_err(TextureMapError::IoError)?;

                memory_reqs = (texd_header.mip_sizes.first().copied().unwrap_or(0x0)
                    + texd_header.mip_sizes.get(1).copied().unwrap_or(0x0))
                    as usize;

                data_size as usize
                    - (TextureMapHeaderV1::size() - 8)
                    - atlas.map(|a| a.size()).unwrap_or(0)
            }
            GlacierGame::HM2 => {
                ...
                data_size as usize
                    - (TextureMapHeaderV2::size())
                    - atlas.map(|a| a.size()).unwrap_or(0)
            }
            GlacierGame::HM3 | GlacierGame::KNT => data.len(),
        };
    ...
    fn pack_internal<W: Write + Seek>(
        ...
        writer
            .write_all(
                match glacier_game {
                    GlacierGame::HM2016 | GlacierGame::HM2 => self
                        .header
                        .iter()
                        .chain(&self.data)
                        .cloned()
                        .collect::<Vec<u8>>(),
                    GlacierGame::HM3 | GlacierGame::KNT => self.data.clone(),
                }
                .as_slice(),
            )
            .map_err(|e| {
                TexturePackerError::PackingError(format!("Unable to pack mipblock1: {e}"))
            })?;
        Ok(())
    }
}
