...
pub struct PrimMeshLinked {
    ...
    #[br(temp)]
    pub _unused: u32, //bone_indices_offset in weighted prims
...
pub enum BoneInfoHolder {
    ...
    Normal(#[bw(args(bone_coli_offset))] BoneInfo),
    #[br(pre_assert(woa_version != WoaVersion::HM2016))]
    Compact(#[bw(args(bone_coli_offset))] CompactBoneInfo),
}

#[binrw::parser(reader)]
fn parse_bone_remap(total_chunks_align: u32) -> BinResult<Vec<u16>> {
    let u64_count = (total_chunks_align + 1).div_ceil(64) as usize;
    let mut bitset = vec![u16::MAX; u64_count * 64];

    let values: Vec<u64> = (0..u64_count)
        .flat_map(|_| u64::read_le(reader))
        .collect::<Vec<_>>();
    ...
    Ok(bitset
        .into_iter()
        .take((total_chunks_align + 1) as usize)
        .collect())
}

fn encode_bone_remap_values(bone_remap: Vec<u16>) -> Vec<u64> {
    ...
    for word in 0..u64_count {
        ...
        for bit in 0..64 {
            ...
            if bone_remap.get(idx).copied().unwrap_or(u16::MAX) != u16::MAX {
                value |= 1u64 << bit;
            }
        }
        out.push(value);
...
impl PrimMeshLinked {
    pub fn bone_remap(&self) -> Vec<Option<usize>> {
        match &self.bone_info {
            BoneInfoHolder::Normal(normal) => normal
                .bone_remap
                .iter()
                .map(|x| {
                    if *x == u8::MAX { None }
                    else { Some(*x as usize) }
                })
                .collect(),
            BoneInfoHolder::Compact(compact) => compact
                .bone_remap
                .iter()
                .map(|x| {
                    if *x == u16::MAX { None }
    ...
    pub fn accel_entries(&self) -> &Vec<BoneAccel> {
        match &self.bone_info {
            BoneInfoHolder::Normal(normal) => &normal.accel_entries,
            BoneInfoHolder::Compact(compact) => &compact.accel_entries,
        }
    }
