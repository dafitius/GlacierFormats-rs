...
pub struct Transform {
    //in glacier this is known as SQV
...
impl From<Transform> for Matrix43 {
    fn from(transform: Transform) -> Self {
        ...
        Self {
            x_axis: Vector3 {
                x: rotation.matrix().m11,
                y: rotation.matrix().m21,
                z: rotation.matrix().m31,
            },
            y_axis: Vector3 {
                x: rotation.matrix().m12,
                y: rotation.matrix().m22,
                z: rotation.matrix().m32,
            },
            z_axis: Vector3 {
                x: rotation.matrix().m13,
                y: rotation.matrix().m23,
                z: rotation.matrix().m33,
            },
            trans: Vector3 {
                x: translation.x,
                y: translation.y,
...
impl From<Matrix43> for Transform {
    fn from(value: Matrix43) -> Self {
        let quaternion = UnitQuaternion::from_matrix(&nalgebra::Matrix3::new(
            value.x_axis.x,
            value.y_axis.x,
            value.z_axis.x,
            value.x_axis.y,
...
impl Matrix43 {
    ...
    pub fn round(&mut self, epsilon: f32) {
        if self.x_axis.x.abs() < epsilon {
            self.x_axis.x = 0.0;
        }
        if self.x_axis.y.abs() < epsilon {
            self.x_axis.y = 0.0;
        }
        if self.x_axis.z.abs() < epsilon {
            self.x_axis.z = 0.0;
        }
        if self.y_axis.x.abs() < epsilon {
            self.y_axis.x = 0.0;
        }
        if self.y_axis.y.abs() < epsilon {
            self.y_axis.y = 0.0;
        }
        if self.y_axis.z.abs() < epsilon {
            self.y_axis.z = 0.0;
        }
        if self.z_axis.x.abs() < epsilon {
            self.z_axis.x = 0.0;
        }
        if self.z_axis.y.abs() < epsilon {
            self.z_axis.y = 0.0;
        }
        if self.z_axis.z.abs() < epsilon {
            self.z_axis.z = 0.0;
        }
        if self.trans.x.abs() < epsilon {
            self.trans.x = 0.0;
        }
        if self.trans.y.abs() < epsilon {
            self.trans.y = 0.0;
        }
        if self.trans.z.abs() < epsilon {
            self.trans.z = 0.0;
        }

        if 1.0 - self.x_axis.x.abs() < epsilon {
            self.x_axis.x = 1.0;
        }
        if 1.0 - self.x_axis.y.abs() < epsilon {
            self.x_axis.y = 1.0;
        }
        if 1.0 - self.x_axis.z.abs() < epsilon {
            self.x_axis.z = 1.0;
        }
        if 1.0 - self.y_axis.x.abs() < epsilon {
            self.y_axis.x = 1.0;
        }
        if 1.0 - self.y_axis.y.abs() < epsilon {
            self.y_axis.y = 1.0;
        }
        if 1.0 - self.y_axis.z.abs() < epsilon {
            self.y_axis.z = 1.0;
        }
        if 1.0 - self.z_axis.x.abs() < epsilon {
            self.z_axis.x = 1.0;
        }
        if 1.0 - self.z_axis.y.abs() < epsilon {
            self.z_axis.y = 1.0;
        }
        if 1.0 - self.z_axis.z.abs() < epsilon {
            self.z_axis.z = 1.0;
        }
        if 1.0 - self.trans.x.abs() < epsilon {
            self.trans.x = 1.0;
        }
        if 1.0 - self.trans.y.abs() < epsilon {
            self.trans.y = 1.0;
        }
        if 1.0 - self.trans.z.abs() < epsilon {
            self.trans.z = 1.0;
        }
    }
    ...
    pub fn flipped_z(&self) -> Self {
        ...
        let flip_z =
            nalgebra::Matrix4::new_nonuniform_scaling(&nalgebra::Vector3::new(1.0, 1.0, -1.0));
        (flip_z * new_mat).into()
    }
}
