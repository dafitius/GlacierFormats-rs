...
impl XteaConfig {
    pub const WOA: Self = Self::Woa;
    pub const KNT: Self = Self::Knt;

    pub const fn key(self) -> [u32; 4] {
        match self {
            Self::Woa => [0x30f95282, 0x1f48c419, 0x295f8548, 0x2a78366d],
            Self::KNT => [0x71482CF0, 0x5FDC4B9F, 0x86CE569D, 0x0509FC1E],
            Self::Custom { key, .. } => key,
        }
    }
    ...
    pub const fn l10n_key(self) -> [u32; 0x4] {
        match self {
            Self::Woa => [0x53527737, 0x7506499E, 0xBD39AEE3, 0xA59E7268],
            Self::Knt => [0x68AC3361, 0x562B4AA0, 0xB9F2771F, 0x28EB3CE7],
            Self::Custom { l10n_key, .. } => l10n_key,
        }
    }
}
...
impl From<XteaConfig> for Xtea {
    fn from(config: XteaConfig) -> Self {
        Self { config }
    }
}

impl Xtea {
    pub const fn new(config: XteaConfig) -> Self {
        Self { config }
    }

    pub const fn config(&self) -> XteaConfig {
        self.config
    }
    ...
    pub fn decrypt_text_file(&self, input_buffer: &[u8]) -> Result<String, XteaError> {
        let payload_start = self.config.header().len() + 4;
        ...
        let xtea = XTEA::new(&self.config.key());
    ...
    pub fn decrypt_string(&self, input_buffer: &[u8]) -> Result<String, XteaError> {
        ...
        let xtea = XTEA::new(&self.config.l10n_key());
        let mut out_buffer = vec![0u8; input.len()];
    ...
    pub fn encrypt_text_file(&self, input_string: String) -> Result<Vec<u8>, XteaError> {
    ...
    pub fn encrypt_string(&self, input_string: String) -> Result<Vec<u8>, XteaError> {
